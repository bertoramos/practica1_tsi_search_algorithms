...
Result = namedtuple('Result', 'node visited_nodes')
...
def graph_search(problem, fringe):
    ...
    while fringe:
        ...
        if problem.goal_test(node.state):
            return Result(node=node, visited_nodes=visited_count)
...
def branch_and_bound_search(problem):
    ...
    return graph_search(problem, BranchBoundQueue())


def heuristic_branch_and_bound_search(problem):
    """Search the shallowest nodes in the search tree first. [p 74]"""
    return graph_search(problem, HeuristicBranchBoundQueue(problem))
